...
st.title("🚚 Plataforma VRP con OR-Tools")

st.markdown("""
Ingrese los datos de los clientes, vehículos y costos para resolver el problema clásico de ruteo de vehículos.
""")

clients_data = st.text_area(
    "Clientes (latitud,longitud,demanda), una por línea:",
    "-33.4500,-70.6667,5\n-33.4550,-70.6700,3\n-33.4600,-70.6650,7\n-33.4650,-70.6600,4",
    height=150
)

num_vehicles = st.number_input("Número de vehículos:", min_value=1, max_value=10, value=2, step=1)
capacities_str = st.text_input("Capacidades de vehículos (separadas por coma):", "10,15")
...
if st.button("Resolver VRP"):
    try:
        ...
        if len(capacities) != num_vehicles:
            st.error("El número de capacidades debe coincidir con el número de vehículos.")
        else:
            routes = solve_vrp(coords, demands, num_vehicles, capacities, cost_per_km)
            if not routes:
                st.error("No se encontró solución. Revisa las capacidades y demandas.")
            else:
                for r in routes:
                    st.write(f"Vehículo {r['vehicle']} -> Ruta: {r['route']}, Distancia: {r['distance_km']:.2f} km, Costo: ${r['cost']:.2f}")
